<?php
/**
 * MS SQL layer for DBO (modified, using sqlsrv extension)
 * 
 ...
 * @package       cake
 * @subpackage    cake.cake.libs.model.datasources.dbo
 * @link          http://blogs.msdn.com/b/brian_swan/archive/2010/03/10/mssql-vs-sqlsrv-what-s-the-difference-part-2.aspx
 */
 ...
 * @var string
 */
	var $description = "MS SQL DBO Driver (using sqlsrv extension)";
...
/**
 * To save last SQL statement (modified)
 * 
 * @deprecated
 * @var string
 * @access private
 ...
 */
	function connect() {
		...
		//config needed to sqlsrv_connect
 ...
 */
	function describe(&$model) {
		...
		$q = "SELECT COLUMN_NAME as Field, DATA_TYPE as Type, COL_LENGTH('" . $table . "', COLUMN_NAME) as Length, IS_NULLABLE As [Null], COLUMN_DEFAULT as [Default], COLUMNPROPERTY(OBJECT_ID('" . $table . "'), COLUMN_NAME, 'IsIdentity') as [Key], NUMERIC_SCALE as Size FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '" . $table . "'";
		
		$cols = $this->fetchAll("SELECT COLUMN_NAME as Field, DATA_TYPE as Type, COL_LENGTH('" . $table . "', COLUMN_NAME) as Length, IS_NULLABLE As [Null], COLUMN_DEFAULT as [Default], COLUMNPROPERTY(OBJECT_ID('" . $table . "'), COLUMN_NAME, 'IsIdentity') as [Key], NUMERIC_SCALE as Size FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '" . $table . "'", $cache, $model);
 ...
 */
	function lastAffected() {
		...
		if ($this->_result) {
			if(sqlsrv_rows_affected($this->_result) === false) {
				//error
				$rows_affected = -999;
			} else if(sqlsrv_rows_affected($this->_result) == -1) {
				//no information available
				$rows_affected = 0;
			} else {
				$rows_affected = sqlsrv_rows_affected($this->_result);
			}
